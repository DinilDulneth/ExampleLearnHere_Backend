    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            if ($row["owner"] == 1) {
                ...
                echo "<strong>Admin:</strong> " . htmlspecialchars($row["message"]) . "<br>";
                echo "<small>" . htmlspecialchars($row["date"]) . " " . htmlspecialchars($row["time"]) . "</small>";
                echo "</div>";
            }
            ...
            if ($row['isNew'] == 1) {
                $updateSql = "UPDATE messageadminteacher SET isNew = 0 WHERE messageID = ?";
...
?>
    ...
    <div class="form-container">
        <form action="teachInsertMessage.php" method="POST">
